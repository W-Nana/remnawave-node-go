...
func TestGetError_NotFound(t *testing.T) {
	_, ok := GetError("INVALID")
	assert.False(t, ok)
}

func TestErrorConstants(t *testing.T) {
	assert.Equal(t, "A001", CodeInternalServerError)
	assert.Equal(t, "A003", CodeUnauthorized)
	assert.Equal(t, "A010", CodeFailedToGetSystemStats)
}
